/*
 16) В физической лаборатории проводится долговременный эксперимент по изучению гравитационного поля Земли. По каналу связи каждую минуту в лабораторию передаётся
 положительное целое число – текущее показание прибора «Сигма 2015». Количество передаваемых чисел в серии известно и равняется N.
 Все числа не превышают 1000. Временем, в течение которого происходит передача, можно пренебречь.
 Необходимо вычислить «бета-значение» серии показаний прибора – максимальное чётное произведение двух показаний, между моментами передачи которых прошло не менее 6 минут.
 Если получить такое произведение не удаётся, ответ считается равным –1. Программа должна быть эффективной по времени и памяти.
*/
...
int main()
{
	...
	*/
	...
	// Т.к. последовательность отсортирована по возрастанию, а нам нужны максимальные значения, то движемся в обратном порядке
	// Не обязательно максимальным произведением будет произведение с самым максимальным числом, все зависит от расположения чисел
	// Условием прерывания будем считать когд макс.произведение для следующего числа (в порядке убывание) окажется меньше или равно имеющегося макс.произведения. 

	// Начальное значение - макс.бета для самого максимального
	i = N - 1;
	maxVal = -1;
	for (j = i - 1; j >= 0; j--) { // пробегаем по числам, которые лежат ранее текущего
		if (abs(masVal[i].curVal - masVal[j].curVal) >= 6) { // если разница номеров в последовательности >= 6
			sumVar = masVal[i].valSort * masVal[j].valSort;
			if ((sumVar % 2) == 0) { // произведение четное
				maxVal = sumVar;
				break;
			}
		}
	...
	if (maxVal == -1) { // если в последовательности нет четных - сразу прерываем
	...
	for (i = N - 2; i >= 0; i--) {
		sumVar = -1;
		for (j = i - 1; j >= 0; j--) { // пробегаем по числам, которые лежат ранее текущего
			if (abs(masVal[i].curVal - masVal[j].curVal) >= 6) { // если разница номеров в последовательности >= 5
				sumVar = masVal[i].valSort * masVal[j].valSort;
				if ((sumVar % 2) == 0) { // произведение четное
					break;
				}
