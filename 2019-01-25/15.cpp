// 15) Напишите программу, находящую для заданной последовательности длины N целых чисел необходимо: 
// минимальное произведение двух её элементов, номера которых различаются не менее чем на 5. Программа должна быть эффективной по времени и памяти.
...
int main()
{
	...
	*/
	...
	// Т.к. последовательность отсортирована по возрастанию, а нам нужны минимальные значения, то движемся в прямом порядке
	// Не обязательно минимальным произведением будет произведение с самым минимальным числом, все зависит от расположения чисел
	// Условием прерывания будем считать когд мин.произведение для следующего числа (в порядке возрастания) окажется больше или равно имеющегося мин.произведения. 

	// Начальное значение - мин.произведение для самого минимального
	i = 0;
	for (j = i + 1; j < N; j++) { // пробегаем по числам, которые лежат далее текущего
	...
	for (i = 1; i < N; i++) {
		...
		if (sumVar >= minVal) { // условие прерывания цикла
			break;
		}
		// новый минимум
		minVal = sumVar;
